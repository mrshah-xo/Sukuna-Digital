...
// ── Shared layout class for all screen root divs ──────────────────
// Mobile: screen manages its own scroll (h-full overflow-y-auto)
// Desktop: content area manages scroll; screen flows naturally
export const SCREEN = "flex flex-col h-full overflow-y-auto min-[834px]:h-auto min-[834px]:overflow-visible min-[834px]:min-h-full";
...
export function SectionLabel({ children }: { children: ReactNode }) {
  return (
    <p
      className="type-caption text-ink-48"
...
export function StatusBadge({ variant, label }: { variant: BadgeVariant; label: string }) {
  ...
  return (
    <span
      ...
      style={{ fontFamily: 'var(--font-body)', fontSize: 12, fontWeight: 400, letterSpacing: '-0.12px', color: '#1d1d1f' }}
    >
      <span
...
export function PrimaryButton({
  onClick, children, disabled, destructive, ghost, fullWidth,
}: {
    ...
    >
      ...
      {active && (
        <div
          ...
          style={{ background: '#DC2626', borderRadius: 4, padding: '2px 8px', fontFamily: 'var(--font-body)', fontSize: 10, fontWeight: 600, letterSpacing: '0.08em' }}
        >
          LIVE
        </div>
      )}
      <div className="absolute bottom-3 right-3" style={{ background: 'rgba(255,255,255,0.85)', borderRadius: 4, padding: '2px 8px', fontFamily: 'var(--font-body)', fontSize: 11, letterSpacing: '-0.12px', color: '#7a7a7a' }}>
        Map pending
      </div>
    </div>
  );
...
// ── ResponderTimeline ─────────────────────────────────────────────
type TimelineStep = { label: string; sublabel?: string; done: boolean; active?: boolean };

export function ResponderTimeline({ steps }: { steps: TimelineStep[] }) {
  return (
        ...
        <li key={i} style={{ display: 'flex', gap: 12 }}>
          ...
          <div style={{ paddingBottom: i < steps.length - 1 ? 20 : 0, paddingTop: 2 }}>
            <p
              ...
              style={{ color: step.done ? '#1d1d1f' : step.active ? '#0066cc' : '#7a7a7a' }}
            >
              {step.label}
            </p>
            {step.sublabel && (
              <p
                ...
                style={{ marginTop: 2, color: step.done ? '#7a7a7a' : step.active ? '#2997ff' : '#e0e0e0' }}
              >
                {step.sublabel}
              </p>
            )}
...
const NAV_ITEMS: { id: ActiveTab; label: string; icon: (active: boolean) => ReactNode }[] = [
  {
    id: 'sos',
    label: 'SOS',
    icon: (a) => (
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
        <path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" stroke={a ? '#0066cc' : '#7a7a7a'} strokeWidth="1.8" strokeLinejoin="round"/>
        <path d="M12 8v4M12 16h.01" stroke={a ? '#0066cc' : '#7a7a7a'} strokeWidth="2" strokeLinecap="round"/>
      </svg>
    ),
  },
  {
    ...
    icon: (a) => (
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
        <circle cx="12" cy="12" r="9" stroke={a ? '#0066cc' : '#7a7a7a'} strokeWidth="1.8"/>
        <path d="M12 7v5l3 3" stroke={a ? '#0066cc' : '#7a7a7a'} strokeWidth="1.8" strokeLinecap="round"/>
      </svg>
    ),
  },
  {
    ...
    icon: (a) => (
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
        <path d="M20 6L9 17L4 12" stroke={a ? '#0066cc' : '#7a7a7a'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
      </svg>
    ),
  },
  {
    ...
    icon: (a) => (
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
        <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7z" stroke={a ? '#0066cc' : '#7a7a7a'} strokeWidth="1.8" strokeLinejoin="round"/>
        <circle cx="12" cy="9" r="2.5" stroke={a ? '#0066cc' : '#7a7a7a'} strokeWidth="1.6"/>
      </svg>
    ),
  },
];

export function BottomNav({ active, onChange, sosState }: { active: ActiveTab; onChange: (t: ActiveTab) => void; sosState: SosState }) {
  const sosLive = sosState === 'active' || sosState === 'ack' || sosState === 'assistance';
  return (
    <nav
      className="w-full bg-canvas border-t border-hairline"
        ...
        {NAV_ITEMS.map(item => {
          ...
          return (
            <button
              ...
              style={{ padding: '10px 0 10px', minHeight: 49, border: 'none', background: 'none', cursor: 'pointer' }}
            ...
            >
              <div className="relative">
                ...
                {item.id === 'sos' && sosLive && (
                  <span
                    className="live-dot absolute -top-0.5 -right-0.5"
                    style={{ width: 8, height: 8, borderRadius: '50%', background: '#DC2626', display: 'block' }}
              ...
              <span
                className="type-nav"
                style={{ color: isActive ? '#0066cc' : '#7a7a7a' }}
              >
                {item.label}
              </span>
            </button>

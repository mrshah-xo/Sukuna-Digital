...
const strings = {
  en: {
    ...
    subtitle: (p: string) => <><span style={{ color: "#7a7a7a" }}>Code sent to </span><span style={{ color: "#1d1d1f", fontWeight: 600 }}>+977 {p}</span></>,
    expiresIn: (m: string, s: string) => <><span style={{ color: "#7a7a7a" }}>Expires in </span><span style={{ fontWeight: 600, fontVariantNumeric: "tabular-nums" }}>{m}:{s}</span></>,
  ...
  ne: {
    ...
    expiresIn: (m: string, s: string) => <><span style={{ fontWeight: 600, fontVariantNumeric: "tabular-nums" }}>{m}:{s}</span> <span style={{ color: "#7a7a7a" }}>मा समाप्त</span></>,
...
export function OTPVerification({ phone, onVerify, onBack, isMobile = false, language }: OTPVerificationProps) {
  ...
  // BACKEND WIRED: uses NextAuth signIn('credentials') with OTP provider
  ...
  const handleResend = async () => {
    ...
    setError(false);
    inputRefs.current[0]?.focus();
    // Resend OTP via the existing API endpoint

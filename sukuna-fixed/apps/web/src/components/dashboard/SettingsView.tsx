...
export type SubPage = 'help' | 'about' | 'terms' | 'privacy' | 'edit_profile' | null;
...
function EditProfileView({ onBack }: { onBack: () => void }) {
  ...
  const [studentClass] = useState(profileData.class);
  const [rollNo] = useState(profileData.rollNo);
  const [email, setEmail] = useState(profileData.email);
  const [phone, setPhone] = useState(profileData.phone);
  const [address, setAddress] = useState(profileData.address);

  const handleSave = () => {
    updateProfile({ name, email, phone, address, profilePicture });
    onBack();
  };
  ...
  return (
    <div className="p-4 md:p-8 max-w-2xl mx-auto min-h-full" style={{ backgroundColor: '#f5f5f7', fontFamily: sf }}>
      ...
      <h1 style={{ fontFamily: sfD, fontSize: '28px', fontWeight: 700, color: '#1d1d1f', letterSpacing: '-0.374px', marginBottom: '24px' }}>Edit Profile</h1>
      
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: '24px' }}>
        <div style={{ position: 'relative', width: '100px', height: '100px', borderRadius: '50%', overflow: 'hidden', border: '3px solid #fff', boxShadow: '0 4px 12px rgba(0,0,0,0.1)', marginBottom: '12px' }}>
          <Image src={profilePicture} alt="Profile" fill sizes="100px" style={{ objectFit: 'cover' }} />
        </div>
        <label style={{ color: '#007AFF', fontSize: '15px', fontWeight: 500, cursor: 'pointer' }}>
          Change Photo
      ...
      <div className="space-y-4">
        <div style={{ backgroundColor: '#fff', padding: '16px', borderRadius: '12px', border: '1px solid #ebebeb' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
            <label style={{ fontSize: '13px', color: '#6e6e73' }}>Name</label>
            <span style={{ fontSize: '11px', color: '#ff3b30' }}>Requires admin permission</span>
          </div>
          <input value={name} disabled style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid #e5e5ea', backgroundColor: '#f5f5f7', fontSize: '16px', outline: 'none', color: '#8e8e93' }} />
        </div>
        <div style={{ backgroundColor: '#fff', padding: '16px', borderRadius: '12px', border: '1px solid #ebebeb' }}>
          <div style={{ display: 'flex', gap: '16px' }}>
            <div style={{ flex: 1 }}>
              <label style={{ display: 'block', fontSize: '13px', color: '#6e6e73', marginBottom: '6px' }}>Class/Section</label>
              <input value={studentClass} disabled style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid #e5e5ea', backgroundColor: '#f5f5f7', fontSize: '16px', outline: 'none', color: '#8e8e93' }} />
            </div>
            <div style={{ flex: 1 }}>
              <label style={{ display: 'block', fontSize: '13px', color: '#6e6e73', marginBottom: '6px' }}>Roll Number</label>
              <input value={rollNo} disabled style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid #e5e5ea', backgroundColor: '#f5f5f7', fontSize: '16px', outline: 'none', color: '#8e8e93' }} />
            </div>
          </div>
        </div>
        <div style={{ backgroundColor: '#fff', padding: '16px', borderRadius: '12px', border: '1px solid #ebebeb' }}>
          <label style={{ display: 'block', fontSize: '13px', color: '#6e6e73', marginBottom: '6px' }}>Email</label>
          <input value={email} onChange={e => setEmail(e.target.value)} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid #d1d1d6', fontSize: '16px', outline: 'none' }} />
        </div>
        <div style={{ backgroundColor: '#fff', padding: '16px', borderRadius: '12px', border: '1px solid #ebebeb' }}>
          <label style={{ display: 'block', fontSize: '13px', color: '#6e6e73', marginBottom: '6px' }}>Phone</label>
          <input value={phone} onChange={e => setPhone(e.target.value)} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid #d1d1d6', fontSize: '16px', outline: 'none' }} />
        </div>
        <div style={{ backgroundColor: '#fff', padding: '16px', borderRadius: '12px', border: '1px solid #ebebeb' }}>
          <label style={{ display: 'block', fontSize: '13px', color: '#6e6e73', marginBottom: '6px' }}>Address</label>
          <input value={address} onChange={e => setAddress(e.target.value)} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid #d1d1d6', fontSize: '16px', outline: 'none' }} />
        </div>
      </div>
      
      <button onClick={handleSave} style={{ width: '100%', marginTop: '32px', padding: '14px', backgroundColor: '#0066cc', color: '#fff', fontSize: '17px', fontWeight: 600, borderRadius: '12px' }}>
        Save Changes
      </button>
    </div>
  );
...
// ─── Main Settings Menu ───────────────────────────────────────────────────────
export default function SettingsView({ initialSubPage }: { initialSubPage?: SubPage }) {
  const { profileData } = useProfile();
  const [subPage, setSubPage] = useState<SubPage>(initialSubPage || null);

  if (subPage === 'edit_profile') return <EditProfileView onBack={() => setSubPage(null)} />;
  ...
  const SECTIONS = [
    {
      title: 'Account',
      items: [
        { icon: <User size={18} />, label: 'Profile',    sub: 'Edit your school profile', color: '#0066cc', bg: '#0066cc12', action: () => setSubPage('edit_profile') },
